...
public class DBConnectionTest {
    public static void main(String[] args) throws Exception {
        ...
        String DB_USER = "mc";
        String DB_PASSWORD = "123qwe!@#QWE";

        Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD); // 데이터베이스의 연결을 얻는다.
        Statement stmt  = conn.createStatement(); // Statement를 생성한다. 명령을 위해선 statement가 필요하다

        String query = "SELECT now()"; // 시스템의 현재 날짜시간을 출력하는 쿼리(query)
        ResultSet rs = stmt.executeQuery(query); // query를 실행한 결과를 rs에 담는다. //ResultSet은  테이블 형태로 데이터를 가져온다
